      ...
      <table className="min-w-full text-sm border border-gray-200 rounded-lg">
        <thead className="bg-gray-50 border-b">
          <tr>
            ...
            <th className="px-6 py-3 text-left font-semibold text-gray-600">Excerpt</th>
            <th className="px-6 py-3 text-left font-semibold text-gray-600">Published</th>
        ...
        <tbody className="divide-y">
          {rows.map((n) => (
            <tr key={n.id ?? n.slug ?? JSON.stringify(n)} className="hover:bg-gray-50 transition">
              <td className="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
                {n.title}
              </td>

              <td className="px-6 py-3 text-gray-700 max-w-xs">
                <p className="line-clamp-2" title={n.excerpt}>
                  {n.excerpt || "-"}
                </p>
              </td>
