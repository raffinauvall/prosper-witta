...
export async function POST(req: Request) {
  try {
    ...
    const { data: product, error: productError } = await supabaseAdmin
      .from("products")
      .insert([{ name, description, display }])
      .select()
      .single();
    ...
    if (msdsFile) {
      ...
      const { error } = await supabaseAdmin.storage.from("documents").upload(msdsPath, msdsFile, {
        upsert: true,
    ...
    if (tdsFile) {
      ...
      const { error } = await supabaseAdmin.storage.from("documents").upload(tdsPath, tdsFile, {
        upsert: true,
